        ...
        protected override async ETTask Run(Session session, C2A_LoginAccount request, A2C_LoginAccount response, Action reply)
        {
            ...
            using (session.AddComponent<SessionLockingComponent>())
            {
                ...
                using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.LoginAccount, request.AccountName.Trim().GetHashCode()))
                {
                    ...
                    var accountInfoList5 = await DataAccess.QueryAccounts(session, request.AccountName.Trim());

                    if (accountInfoList5 != null && accountInfoList5.Count > 0)
                    {
                        account = accountInfoList5[accountInfoList5.Count - 1];

                        #region 账号存在时，增加ABoardGame和SandToyTable表

                        IABoardGame iABoardGame = DataAccess.InsertABoardGame();
                        ...
                        await iABoardGame.Insert(aBoardGame, session);

                        var aBoardGameList2 = await DataAccess.QueryABoardGame(session, account.AccountID);

                        if (aBoardGameList2 != null && aBoardGameList2.Count > 0)
                        {
                            //更新Account表
                            DateTime.TryParse(aBoardGameList2[aBoardGameList2.Count - 1].MakeTime, out var ultimate);
                            DateTime.TryParse(aBoardGameList2[aBoardGameList2.Count - 2].MakeTime, out var penultimate);
                            System.TimeSpan t = ultimate - penultimate;

                            //更新Account表中IntervalTime
                            IAccount iaccount1 = DataAccess.InsertAccount();
                            await iaccount1.UpdateFieldContext(account, session, "IntervalTime", t.ToString());
                            //更新Account表中UseOfNumber
                            await iaccount1.UpdateFieldContext(account, session, "UseOfNumber", aBoardGameList2.Count.ToString());

                            //保存SandToyTable表
                            ISandToyTable iSandToyTable = DataAccess.InsertSandToyTable();
                            sandToyTable = session.AddChild<SandToyTable>();
                            sandToyTable.Number = 0;
                            sandToyTable.BelongABoardGameID = aBoardGameList2[aBoardGameList2.Count - 1].GameID;
                            await DBManagerComponent.Instance.GetZoneDB(session.DomainZone()).Save<SandToyTable>(sandToyTable);
                            await iSandToyTable.Insert(sandToyTable, session);
                        }
                        ...
                        // var aBoardGameList3 = await DataAccess.QueryABoardGame(session, account.AccountID);
                        //
                        // if (aBoardGameList3 != null && aBoardGameList3.Count > 0)
                        // {
                        //     IABoardGame iABoardGame = DataAccess.InsertABoardGame();
                        //     await iABoardGame.Remove(session, aBoardGameList3[aBoardGameList3.Count - 1]);
                        // }
                    ...
                    {
                        IAccount iaccount = DataAccess.InsertAccount();
                        account = session.AddChild<Account>();
                        account.AccountName = request.AccountName.Trim();
                        account.Password = request.Password;
                        account.AccountType = (int) AccountType.General;
                        account.AccountID = RandomHelper.RandInt64();
                        await iaccount.Insert(account, session);
